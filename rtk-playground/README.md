...
Третья настройка — `ignoreCommand`:

```json
"ignoreCommand": "git diff --quiet HEAD^ HEAD ."
```

В репозитории живут два независимых приложения (`rtk-playground/` и `qr-generator/`),
каждое — отдельный проект Vercel со своим корневым каталогом. Без этой строки любой push
в `master` запускал бы обе сборки, хотя изменился только один каталог. Vercel выполняет
команду из корневого каталога проекта: код выхода **0 означает «пропустить сборку»**,
а `git diff --quiet HEAD^ HEAD .` даёт 0 ровно тогда, когда в этом каталоге ничего
не менялось.

Если `HEAD^` недоступен (первый деплой, неглубокий клон), команда завершится с ненулевым
кодом и сборка просто пойдёт. Отказ безопасный: в сомнительной ситуации собираем,
а не пропускаем молча.

> ⚠️ В `vercel.json` **нельзя добавлять свои ключи** — схема строгая и отвергает всё
> постороннее, включая попытки оставить комментарий ключом вида `"//..."`. Сборка падает
> с `should NOT have additional property`. Поэтому пояснения живут здесь, а не в конфиге.
