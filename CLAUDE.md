...
Дожо — это веб-приложение, развёрнутое на https://ts-dojo-beta.vercel.app. Компилятор
TypeScript 5.9.3 работает прямо в браузере (monaco-editor несёт ровно эту версию), поэтому
ошибки, их тексты и выведенные типы — настоящие, а не имитация. Уроки описываются данными
в `src/data/lessons/`, а не вёрсткой.

**Все 37 уроков программы написаны.** Содержимое каждого проверяется автоматически: валидатор
прогоняет каждый сниппет через компилятор и сверяет с заявленным вердиктом, эталонные решения
обязаны проходить собственные проверки задания, стартовый код — падать заявленными кодами.
Перед правкой урока запусти `node --experimental-strip-types scripts/sync-expectations.ts <slug>`
и сверь напечатанные диагностики с текстом вердикта, затем `npm test`.
